...
//                    JOptionPane.INFORMATION_MESSAGE);
	 ...
	 * @throws IOException
	 */
	public synchronized void uploadContinue(String localFilePath, String serverRoute) throws IOException {
		// 创建数据缓冲区，用于上传数据
...
//                response = readLine();
				if (response.startsWith("226")) {
					JOptionPane.showConfirmDialog(null, " 文件下载成功！", " 下载信息", JOptionPane.CLOSED_OPTION,
							JOptionPane.INFORMATION_MESSAGE);
				}
			}
		}
...
//    }
	...
	public boolean deleteFile(String FilePath) throws IOException {
		...
		if (response.startsWith("250")) {
			...
			JOptionPane.showConfirmDialog(null, "删除文件" + FilePath + "  成功！！", " 删除文件", JOptionPane.CLOSED_OPTION,
	...
	public boolean rmdir(String dirName) throws IOException {
		...
		if (!response.startsWith("250")) { // FTP命令发送过程发生异常
			...
			JOptionPane.showConfirmDialog(null, "删除目录" + dirName + "  成功！！", " 删除目录", JOptionPane.CLOSED_OPTION,
					JOptionPane.INFORMATION_MESSAGE); // 成功删除目录
			return true;
		}
		return false;
...
//用于发送命令
			...
			while (!timeOut) {
				try {
					...
					if (!response.startsWith("200")) { // 200-NOOP OK
						timeOut = true;
						JOptionPane.showConfirmDialog(null, " 连接已断开！", " 连接信息", JOptionPane.CLOSED_OPTION,
								JOptionPane.INFORMATION_MESSAGE);
						break;
					}
	 ...
	 * @throws IOException
	 */
	public synchronized long downloadPart(long start, long end, String remoteFilePath, String localFileDir, int part)
			throws IOException {
